/* Diamond Club 99 — secure member authentication client. */
(function () {
  const SESSION_KEY = 'dc_member_user';
  let cached = null;
  ...
  const DCAuth = {
    ...
    currentSession() { return cached; },
    async login(memberId, password, remember) {
      const data = await api('/auth/member/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ memberId, password, remember }) });
      save(data.user, remember);
      return data;
    },
    async validate() {
      try { const data = await api('/auth/me'); if (data.role !== 'member') throw new Error(); save(data.user, true); return data.user; }
      catch { clear(); return null; }
    },
    async logout() { try { await api('/auth/logout', { method: 'POST' }); } finally { clear(); } },
    requireAuth() {
      if (!cached) { location.replace('login.html?redirect=' + encodeURIComponent(location.pathname.split('/').pop())); return; }
      this.validate().then(user => { if (!user) location.replace('login.html?redirect=' + encodeURIComponent(location.pathname.split('/').pop())); });
    }
  };
